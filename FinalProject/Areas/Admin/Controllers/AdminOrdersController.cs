...
namespace WebShop.Areas.Admin.Controllers
{
    ...
    public class AdminOrdersController : Controller
    {
        ...
        public AdminOrdersController(FinalProjectContext context, INotyfService notyfService)
        {
            _context = context;
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (donhang != null)
                    {
                        ...
                        if (donhang.TransactStatusId == 1004) donhang.Deleted = true; 
                    }
        ...
        public async Task<IActionResult> Edit(int id, [Bind("OrderId,CustomerId,OrderDate,ShipDate,TransactStatusId,Deleted,Paid,PaymentDate,Totalmoney,PaymentId,Note,Address,LocationId,District,Ward")] Order order)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                _notyfService.Success("Chỉnh sửa đơn hàng thành công!");
                return RedirectToAction(nameof(Index));
            }
            ViewData["CustomerId"] = new SelectList(_context.Customers, "CustomerId", "CustomerId", order.CustomerId);
            ViewData["TransactStatusId"] = new SelectList(_context.TransactStatuses, "TransactStatusId", "Status", order.TransactStatusId);
        ...
        {
            var order = await _context.Orders.FindAsync(id);
            var orderdetail = await _context.OrderDetails.AsNoTracking().SingleOrDefaultAsync(x=> x.OrderId == id);
